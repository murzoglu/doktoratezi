#!/usr/bin/env python3
"""PreToolUse hook (Katman 1: deterministik kapı / veri-yönetişimi guardrail).

Bash komutlarını çalışmadan ÖNCE yakalar; yıkıcı işlemleri ve korumalı
çalışma-verisi/credential yollarına dokunan komutları reddeder. Dosya-aracı
erişimi ayrıca `.claude/settings.json` içindeki `permissions.deny`
kurallarıyla kapatılır; bu hook Bash'e odaklanır.

Codex ikizi: .codex/hooks/pre_tool_use_policy.py — politika değişirse iki
ağaç + tests/test_claude_hooks.py birlikte güncellenir.
