...
paths <- save_apa_table_bundle(bundle, "outputs/tables")
manifest <- apa_table_manifest(paths, bundle)
manifest_path <- save_apa_table_csv(manifest, "outputs/tables/apa_sprint_a_table_manifest.csv")

if (!all(manifest$exists) || any(is.na(manifest$bytes)) || any(manifest$bytes <= 0)) {
  stop("APA table audit failed: one or more output files are missing or empty", call. = FALSE)
}

cat(sprintf(
  "APA Sprint A table audit passed: tables=%d, bytes=%s\n",
  nrow(manifest),
