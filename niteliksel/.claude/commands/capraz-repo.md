---
description: Karma tez / joint display öncesi iki-kol köprü durumunu üretir ve yorumlar
allowed-tools: Bash(./dmnitel *), Read
---

## Köprü raporu (otomatik üretildi)

!`./dmnitel cross-repo-status --output 07_reports/cross_repo_thesis_bridge_status.md && echo "yazıldı: 07_reports/cross_repo_thesis_bridge_status.md"`

## Görev

1. `07_reports/cross_repo_thesis_bridge_status.md` dosyasını oku ve karma tez yazımı
   için kritik durumları özetle: hangi kanonik kaynaklar hazır, hangileri eksik.
2. Kanıt ayrımı disiplinini uygula: nitel tema ≠ nicel estimate; nicel sonuç nitel
   bulgunun nedensel kanıtı olarak sunulmaz; joint display katmanı iki kolu yan yana
   getirir ve kanıt türlerini açıkça ayırır.
3. Tez bölümü yazımına geçilecekse ana merkezin
   `/workspaces/T1DM-Tez/tez-yazim` olduğunu ve oradaki bölüm
   briefinin (`03_bolum-hazirlik/`) açılması gerektiğini belirt.
