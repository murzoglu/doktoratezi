...
NUMERIC_CLAIM = re.compile(r"\b\d+(?:[.,]\d+)?\s?%|\b\d{4}\b|\$\s?\d|\b\d+(?:[.,]\d+)?\s?(mg|ml|kg|mmol|patients|hastada|aile(?:den)?|görüşme|gorusme|kod|(?:makro\s+)?tema|madde(?:si)?|cocuk|çocuk|katilimci|katılımcı|satir|satır|sutun|sütun)\b", re.IGNORECASE)
